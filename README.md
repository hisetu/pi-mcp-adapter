...
The optional result archive preserves raw MCP `CallToolResult` payloads before `outputGuard`, while the separate `mcpCache` tool provides explicit namespace-based read-through caching.

- Archive and cache are disabled by default.
- Cache hits require a stable namespace, explicit arguments, TTL validation, and an exact tool allowlist.
- `prefer-cache` uses a valid hit or calls live on miss; `cache-only` never calls live; `refresh` always calls live.
- Direct `figma_*` calls can be archived but do not create reusable pointers because they have no namespace.
- Cached evidence must not be presented as current live verification.
